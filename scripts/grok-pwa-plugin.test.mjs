...
// Hermetic ctx: keep tests independent of this workspace's baked branding
// (src/lib/og/site.json, public/og.*), which otherwise leaks in as defaults.
...
test("platform chrome overwrites share-card metas and always sets og:title", () => {
  ...
  const out = injectGrokPwaHead(html, { appName: "Wild Race", site: {}, cwd: EMPTY_ROOT });
...
test("streaming injector matches </HEAD> case-insensitively", () => {
  const injector = createHeadInjector({ appName: "Wild Race", site: {}, cwd: EMPTY_ROOT });
  const chunks = [
